<script lang="ts">
	import { connection } from '$lib/fuel';
	...
	function purchase() {
		connection.ensureConnected().then(async ($connection) => {
			if ($connection.wallet) {
				const address = await connection.provider.currentAccount();
				if (!address) {
					throw new Error('No current account found');
				}
				connection.provider
					.sendTransaction(address, {} as any)
					.then(() => {
