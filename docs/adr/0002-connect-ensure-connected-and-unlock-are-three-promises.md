...
`ensureConnected()`'s reconnect reuses the connected mechanism, address included, and that address is a demand: if the user unlocks with a different account selected, the attempt fails with `could not find address` and the wallet is torn down. Reusing it is what keeps an ordinary unlock from bouncing a multi-account user into `ChooseWalletAccount`, so the fix is to treat a REUSED address as a preference while a CALLER-SUPPLIED one stays a demand, which needs a way to tell those apart that does not exist yet. Recorded as a characterization test and in `work/notes/observations`.
