...
- `set` enforces the sibling invariant the same way: a state with no `wallet` tears the live wallet down, so the wrapper cannot keep routing (or signing) for a state that shows none. `WaitingForWalletConnection` is the one exception, being `connect`'s own in-progress step. That rule used to live at eleven call sites and was missing from two of them.
