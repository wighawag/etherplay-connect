...
contract SignatureUtilsTest is Test {
    ...
    /// @notice Both `v` encodings are accepted: 27/28 and the raw 0/1 recovery
    /// bit, because signers differ on which they emit.
    ///
    /// The branch that normalises the raw form is otherwise never taken by any
    /// test here, since `vm.sign` returns 27/28, so it would sit uncovered in a
    /// library whose whole job is to be strict about signatures.
    function test_recover_acceptsTheRawRecoveryBitEncoding() public view {
        bytes32 digest = harness.textDigest(bytes("hello"));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(signerKey, digest);

        assertEq(
            harness.recover(digest, abi.encodePacked(r, s, v - 27)),
            _signer()
        );
    }

    /// @notice The honest consequence of accepting both, pinned rather than left
    /// to be discovered: one authorisation has TWO accepted byte strings.
    ///
    /// Harmless here, because nothing in this package identifies a signature by
    /// its bytes. It is asserted so that a caller who does key on them - and who
    /// therefore has to normalise `v` first - meets the fact in a test rather
    /// than in production.
    function test_recover_bothVEncodingsAreOneAuthorisation() public view {
        bytes32 digest = harness.textDigest(bytes("hello"));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(signerKey, digest);

        bytes memory prefixed = abi.encodePacked(r, s, v);
        bytes memory raw = abi.encodePacked(r, s, v - 27);

        assertTrue(keccak256(prefixed) != keccak256(raw));
        assertEq(
            harness.recover(digest, prefixed),
            harness.recover(digest, raw)
        );
    }
    ...
    function test_recover_rejectsAnImpossibleV() public {
        ...
        // 2 normalises to 29, so it is rejected by the same check rather than
        // being mistaken for a third recovery bit.
        vm.expectRevert(SignatureUtils.MalformedSignature.selector);
        harness.recover(digest, abi.encodePacked(r, s, uint8(2)));
    }
