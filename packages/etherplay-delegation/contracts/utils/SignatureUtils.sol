...
library SignatureUtils {
    ...
    ///
    /// `v` IS DELIBERATELY NOT CANONICALISED, and that qualifies the second
    /// point above rather than extending it. Both encodings in circulation are
    /// accepted - the 27/28 of `eth_sign` and the raw 0/1 recovery bit - because
    /// signers genuinely differ on which they emit, and refusing half of them
    /// buys nothing. So one authorisation does still have two accepted byte
    /// strings, differing in the last byte. That is safe HERE because nothing in
    /// this package identifies a signature by its bytes: the withdrawn state is
    /// keyed by (owner, delegate), and a credential presented twice only
    /// re-asserts what is already true. A caller that DOES key on signature
    /// bytes has to normalise `v` itself before doing so.
    ///
