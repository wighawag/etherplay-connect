---
'@etherplay/delegation': minor
---

New package: `@etherplay/delegation`, the onchain half of delegation, shipped as one feature with three faces - the Solidity library (`contracts/`), the TypeScript builder for the message it verifies, and the ABI - kept in agreement by a shared `vectors.json` that both languages are tested against.
